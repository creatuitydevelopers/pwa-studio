...
export const getReviewFetchUrl = products => `${URL_ADDRESS}?passkey=${PASS_KEY}&apiversion=${API_VERSION}&resource.r=statistics&stats.r=reviews&filter.r=productid:eq:${products}`;
...
export const getRatingsForProducts = (reviews, showAverage, placeholder) => {
    ...
    return reviews.map(results => {
        const avgRating = results.ProductStatistics.ReviewStatistics.AverageOverallRating;
        const overallRating = results.ProductStatistics.ReviewStatistics.OverallRatingRange;
        return {
