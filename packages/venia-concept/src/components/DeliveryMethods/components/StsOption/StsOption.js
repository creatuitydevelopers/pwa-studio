...
import { getVstsMethodCode, getStstMethodCode, isCurrentStoreEnabledForSts, getStoreListForStsMethod, getDefaultRadius } from 'src/models/DeliveryMethods';
...
class StsOption extends Component {
    ...
    handleMethodChange = event => {
        ...
        !!store ? this.props.onChange(event.target.value, store) : this.setState({ modalOpen: true });
    };
    ...
    get deliveryMeessage() {
        const { methodCode, methodInfo, classes, currentStore, selectedStore } = this.props;
        const store = !!selectedStore ? selectedStore : currentStore;

        if (methodCode == getVstsMethodCode()) {
            return (!!methodInfo.vsts_lead_time &&
                <span>
                    <span className={classes.separator}>|</span>
                    <span className={classes.positiveMessage}>{methodInfo.vsts_lead_time}</span>
                </span>);
        }
        ...
        return (<span>
            ...
            <span className={classes.positiveMessage}>
                Estimated delivery in {store.dc_minimum_days} to {store.dc_maximum_days} business days
                    </span>
        </span>);
    }
    ...
    get inventoryMessage() {
        ...
        if (isCurrentStoreEnabledForSts({ allowedStores: methodInfo.enabled, storeNumber: store.store_number })) {
            return (<span>
                <Icon name="map-pin" className={classes.pinIcon} />
                <span className={classes.storeName}>{store.company_name}</span>
                {this.deliveryMeessage}
            </span>);

        }

        return (<span className={classes.alertMessage}>
            <Icon name="map-pin" className={classes.pinIcon} />
    ...
    get detailsRender() {
        const { methodInfo, currentStore, selectedStore, allStores, classes } = this.props;
        ...
        return (<div>
            ...
            <p>
                <Button size={`small`}
                    genre={`empty`}
                    onClick={this.handleOpenModalClick}>{!!store ? `Choose Another Store` : `Select store`}</Button>
            </p>
            <Modal open={modalOpen}
                ...
                focusTrapped>
                <h2 className={classes.modalHeader}>Check Availability at Stores</h2>
                <SearchForm
